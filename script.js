...
// Get Filename Identifier (Operation Number + Date)
function getFilenameIdentifier() {
  ...
  for (const header of headers) {
    ...
    if (txt === 'رقم التشغيل') {
      const input = header.parentElement.querySelector('input');
      if (input && input.value.trim()) opNum = input.value.trim();
    } else if (txt === 'التاريخ') {
      const input = header.parentElement.querySelector('input');
      if (input && input.value.trim()) dateVal = input.value.trim();
    }
  }
  return `${opNum}_${dateVal}`.replace(/[^a-zA-Z0-9\u0600-\u06FF_\-]/g, '_');
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function validateFullForm() {
    ...
    const headerContainer = document.querySelector('.mb-6.print\\:mb-2');
    if (headerContainer) {
      const fields = headerContainer.querySelectorAll('.flex.bg-white');
      for (const field of fields) {
        const labelEl = field.querySelector('.bg-gray-100');
        const input = field.querySelector('input');
        
        if (labelEl && input) {
          const label = labelEl.textContent.trim();
          // جعل حقل رقم التشغيل (وباقي الحقول الأساسية) إلزامياً
          if (!input.value || input.value.trim() === '') {
            return { name: label, section: 'معلومات أساسية', element: input };
          }
        }
      }
    }
    ...
    for (let i = 0; i < itemsToCheck.length; i++) {
       ...
       if (isMobile) {
         ...
         if (headerEl) {
            ...
            sectionName = clone.innerText.trim();
         } else {
            sectionName = '';
         }
       }
  ...
  function validateAndAction(action) {
     ...
     if (error) {
        // فتح القسم المطوي تلقائياً إذا كان الحقل بداخله
        let curr = error.element.parentElement;
        while (curr && curr.tagName !== 'BODY') {
           if (curr.classList.contains('hidden')) {
              const prev = curr.previousElementSibling;
              // الحالة 1: العنوان مباشرة قبل المحتوى (التواقيع أو محتوى الجوال)
              if (prev && prev.classList.contains('bg-gray-100') && prev.classList.contains('cursor-pointer')) {
                 prev.click();
                 break;
              }
              // الحالة 2: قسم الملاحظات في الجوال (عنوان -> محتوى -> ملاحظات)
              if (prev && prev.classList.contains('divide-y')) {
                 const header = prev.previousElementSibling;
                 if (header && header.classList.contains('bg-gray-100') && header.classList.contains('cursor-pointer')) {
                    header.click();
                    break;
                 }
              }
  ...
  if (printBtn) printBtn.addEventListener('click', () => validateAndAction(() => {
    ...
    document.title = getFilenameIdentifier();
    window.print();
  ...
  function openSignatureModalFor(canvas) {
    ...
    setTimeout(() => {
      ...
      if (activeSignatureBox) {
        ...
        const srcW = activeSignatureBox.width;
        const srcH = activeSignatureBox.height;
        const dstW = signatureModalCanvas.width;
        const dstH = signatureModalCanvas.height;
        const scale = Math.min(dstW / srcW, dstH / srcH);
        const newW = srcW * scale;
        const newH = srcH * scale;
        const offX = (dstW - newW) / 2;
        const offY = (dstH - newH) / 2;

        ctx.drawImage(activeSignatureBox, 0, 0, srcW, srcH, offX, offY, newW, newH);
      }
  ...
  if (signatureSaveBtn && signatureModalCanvas) {
    signatureSaveBtn.addEventListener('click', () => {
      ...
      // Calculate aspect ratio to prevent stretching
      const srcW = signatureModalCanvas.width;
      const srcH = signatureModalCanvas.height;
      const dstW = activeSignatureBox.width;
      const dstH = activeSignatureBox.height;
      const scale = Math.min(dstW / srcW, dstH / srcH);
      const newW = srcW * scale;
      const newH = srcH * scale;
      const offX = (dstW - newW) / 2;
      ...
      destCtx.drawImage(signatureModalCanvas, 0, 0, srcW, srcH, offX, offY, newW, newH);
      closeSignatureModal();
  ...
  async function exportToPDF() {
    try {
      ...
      // جعل خلفية التوقيع شفافة أثناء التصدير لتبدو أكثر احترافية
      const sigCanvases = document.querySelectorAll('canvas[title="انقر هنا للتوقيع"]');
      sigCanvases.forEach(c => {
        c.classList.remove('bg-white', 'hover:bg-gray-50');
      });
      ...
      // استعادة خلفية التوقيع
      sigCanvases.forEach(c => {
        c.classList.add('bg-white', 'hover:bg-gray-50');
      });
      ...
      // Save with Operation Number and Date
      const filename = getFilenameIdentifier();
  ...
  // Clear Form
  function clearForm() {
    if (!confirm('هل أنت متأكد من رغبتك في مسح جميع بيانات النموذج؟ لا يمكن التراجع عن هذا الإجراء.')) return;
    
    // 1. Clear local storage
    localStorage.removeItem(STORAGE_KEY);
    
    // 2. Reset all input fields to their initial state
    ...
    document.querySelectorAll('input, textarea, select').forEach(el => {
      if (el.type === 'checkbox' || el.type === 'radio') {
        ...
        el.selectedIndex = 0;
        // Manually trigger visual update for empty selects
        if (typeof updateSelectVisualState === 'function') {
          updateSelectVisualState(el);
        }
      } else if (el.type === 'date') {
        el.value = today; // Reset date to today for a fresh start
      } else {
        el.value = ''; // Clear text inputs and textareas
      }
    });
    
    // 3. Clear all signature canvases on the form
    document.querySelectorAll('canvas[title="انقر هنا للتوقيع"]').forEach(canvas => {
      const ctx = canvas.getContext('2d', { willReadFrequently: true });
      ctx.clearRect(0, 0, canvas.width, canvas.height);
    });
    
    // 4. Clear damage markers on the bus diagram
    damageMarkers = [];
    renderPrintMarkers();
    
    // 5. Recalculate all section scores and update notes (this will reset them)
    document.querySelectorAll('table').forEach(updateTableStatus);
    
    // 6. Reset the main progress bar
    updateProgressBar();
    
    // 7. Show confirmation
    showToast('✅ تم مسح جميع الحقول بنجاح.', 'success');
  }
  ...
  function updateTableStatus(table) {
    ...
    if (textarea) {
      ...
      } else if (evaluations.length < totalItems && evaluations.length > 0) {
      ...
      data.signatures?.forEach(item => {
         if (sigCanvases[item.index] && item.data) {
            ...
            img.onload = () => {
               ...
               const cvs = sigCanvases[item.index];
               // مسح الكانفاس ورسم الصورة مع الحفاظ على نسبة الأبعاد
               ctx.clearRect(0, 0, cvs.width, cvs.height);
               
               const scale = Math.min(cvs.width / img.width, cvs.height / img.height);
               const newW = img.width * scale;
               const newH = img.height * scale;
               const offX = (cvs.width - newW) / 2;
               const offY = (cvs.height - newH) / 2;
               
               ctx.drawImage(img, 0, 0, img.width, img.height, offX, offY, newW, newH);
            };
            img.src = item.data;
         }
      });
  ...
  // جعل خلايا الجدول (اجتياز/فشل) قابلة للنقر لتسهيل التحديد
  document.addEventListener('click', (e) => {
    const td = e.target.closest('td');
    if (!td) return;
    
    // التحقق من أننا في الجدول وفي العمود الثاني أو الثالث
    const tr = td.parentElement;
    if (!tr || !tr.closest('table')) return;
    
    const index = Array.from(tr.children).indexOf(td);
    if (index === 1 || index === 2) { // 1=اجتياز, 2=فشل
  ...
  function setupEnhancedUI() {
    // 1. إضافة زر "تحديد الكل سليم" (Desktop & Mobile)
    // ------------------------------------------------
    const sections = [
      { name: 'صيانة', selectorDesktop: 'th:contains("صيانة")', selectorMobile: '.lg\\:hidden .bg-gray-100:contains("صيانة")' },
      { name: 'حوادث وتأمين', selectorDesktop: 'th:contains("حوادث وتأمين")', selectorMobile: '.lg\\:hidden .bg-gray-100:contains("حوادث وتأمين")' },
      { name: 'الأسطول', selectorDesktop: 'th:contains("الأسطول")', selectorMobile: '.lg\\:hidden .bg-gray-100:contains("الأسطول")' },
      { name: 'السائق', selectorDesktop: 'th:contains("السائق")', selectorMobile: '.lg\\:hidden .bg-gray-100:contains("السائق")' }
    ];

    // دالة مساعدة للبحث عن العناصر بالنص (لأن CSS لا يدعم :contains)
    const findHeader = (selector, text) => {
      const els = document.querySelectorAll(selector.split(':contains')[0]);
      return Array.from(els).find(el => el.textContent.includes(text));
    };

    // دالة لإنشاء زر مسح القسم
    const createClearSectionBtn = (container, isMobile) => {
      const btn = document.createElement('button');
      btn.className = `clear-section-btn bg-gray-200 hover:bg-gray-300 text-gray-800 text-[10px] px-2 py-1 rounded border border-gray-400 font-bold transition-colors mx-1 no-print ${isMobile ? '' : 'float-left'}`;
      btn.innerHTML = '🔄 مسح';
      btn.type = 'button';
      btn.title = 'مسح جميع البيانات في هذا القسم فقط';

      btn.addEventListener('click', (e) => {
        e.stopPropagation(); // منع تفعيل الأكورديون عند النقر
        if (!confirm('هل أنت متأكد من رغبتك في مسح بيانات هذا القسم؟')) return;

        const sectionContainer = isMobile ? container.parentElement : container.closest('table');
        if (!sectionContainer) return;

        // 1. إعادة تعيين جميع حقول الإدخال في القسم
        sectionContainer.querySelectorAll('select, input[type="checkbox"]').forEach(el => {
        ...
        // 2. العثور على الجدول المقابل وتحديث حالته (لحساب الدرجات والملاحظات)
        const tableToUpdate = isMobile 
            ? Array.from(document.querySelectorAll('table')).find(table => {
                const th = table.querySelector('th[colspan]');
                const sectionName = container.textContent.replace(/✅.*|🔄.*/, '').trim();
                return th && th.textContent.includes(sectionName);
              })
            : sectionContainer;
        
        if (tableToUpdate) {
            updateTableStatus(tableToUpdate);
        }
        
        showToast(`تم مسح بيانات القسم بنجاح`, 'success');
      });

      return btn;
    };

    // دالة لإنشاء زر التحديد السريع
    const createQuickPassBtn = (container, isMobile) => {
      const btn = document.createElement('button');
      btn.className = `quick-pass-btn bg-green-100 hover:bg-green-200 text-green-800 text-[10px] px-2 py-1 rounded border border-green-300 font-bold transition-colors mx-2 no-print ${isMobile ? 'mr-auto' : 'float-left'}`;
      btn.innerHTML = '✅ الكل سليم';
      btn.type = 'button';
      btn.title = 'تحديد جميع البنود في هذا القسم كـ "اجتياز"';
      
      btn.addEventListener('click', (e) => {
        e.stopPropagation(); // منع تفعيل الأكورديون عند النقر
        if(!confirm('هل تريد تحديد جميع البنود الفارغة في هذا القسم كـ "اجتياز"؟ (سيتم تجاهل البنود التي تم تقييمها بالفعل)')) return;
        
        // تحديد الحاوية الأب (الجدول أو البطاقة)
        const sectionContainer = isMobile ? container.parentElement : container.closest('table');
        if (!sectionContainer) return;

        // 1. تعبئة القوائم المنسدلة الفارغة فقط بالقيم المثالية
        const selects = sectionContainer.querySelectorAll('select');
        selects.forEach(select => {
          // فقط إذا كان الحقل فارغاً
          if (!select.value) {
            let bestOption = null;
            for (const option of select.options) {
              if (PERFECT_OPTIONS.includes(option.value)) {
        ...
        // 2. تفعيل مربعات الاختيار المستقلة (التي لم تتأثر بالقوائم ولم يتم تقييمها)
        const checkboxes = sectionContainer.querySelectorAll('input[type="checkbox"]');
        checkboxes.forEach((cb, index) => {
          const failCb = checkboxes[index + 1];
          // يتم تفعيل "اجتياز" فقط إذا لم يتم تقييم البند (لا اجتياز ولا فشل)
          if (index % 2 === 0 && !cb.checked && !cb.disabled && failCb && !failCb.checked) {
            cb.click();
          }
        });
        showToast('تم تعبئة البنود الفارغة بنجاح', 'success');
      });
      
      return btn;
    };

    // أيقونات الأقسام (رموز حديثة)
    const sectionIcons = {
      'صيانة': '🛠️',
      'حوادث وتأمين': '🚨',
      'الأسطول': '🚍',
    ...
    // تطبيق الزر على العناوين
    const sectionNames = ['صيانة', 'حوادث وتأمين', 'الأسطول', 'السائق'];
    
    // Desktop Headers
    document.querySelectorAll('table thead th[colspan]').forEach(th => {
      const sectionName = sectionNames.find(n => th.textContent.includes(n));
      if (sectionName) {
        // إضافة الأيقونة
        if (sectionIcons[sectionName]) {
          const iconSpan = document.createElement('span');
          iconSpan.textContent = sectionIcons[sectionName];
          iconSpan.className = 'ml-2 inline-block text-xl align-middle';
          th.insertBefore(iconSpan, th.firstChild);
        }

        // الأزرار تطفو لليسار، لذا ترتيب الإضافة يعكس الترتيب البصري
        th.appendChild(createClearSectionBtn(th, false));
        th.appendChild(createQuickPassBtn(th, false));
      }
    });

    // Mobile Headers & Accordion
    const isMobile = window.matchMedia('(max-width: 1279px)').matches;
    
    document.querySelectorAll('.lg\\:hidden .bg-gray-100.text-center').forEach(header => {
      const sectionName = sectionNames.find(n => header.textContent.includes(n));
      if (sectionName) {
        // 1. تحويل العنوان ليكون مرناً (Flex)
        header.classList.remove('text-center');
        header.classList.add('flex', 'items-center', 'justify-between', 'cursor-pointer', 'select-none');
        // حجم النص يتم التحكم به من CSS
        
        // 2. إنشاء حاوية للأزرار لتجميعها على اليسار
        const btnContainer = document.createElement('div');
        btnContainer.className = 'flex items-center gap-2 mr-auto no-print';
        
        btnContainer.appendChild(createQuickPassBtn(header, true));
        btnContainer.appendChild(createClearSectionBtn(header, true));
        
        // إزالة التموضع الفردي من زر التحديد السريع
        btnContainer.querySelector('.quick-pass-btn').classList.remove('mr-auto');
        header.appendChild(btnContainer);

        // 3. إضافة أيقونة السهم للأكورديون
        const arrow = document.createElement('span');
        arrow.innerHTML = '▼';
        arrow.className = 'text-gray-500 text-xs transition-transform duration-300 transform ml-2';
        ...
        if (sectionIcons[sectionName]) {
          const iconSpan = document.createElement('span');
          iconSpan.textContent = sectionIcons[sectionName];
          iconSpan.className = 'ml-2 inline-block text-xl';
          header.insertBefore(iconSpan, arrow.nextSibling);
        }

        // تطبيق الطي الافتراضي فقط على الأجهزة المحمولة
        const contentDiv = header.nextElementSibling; // div.divide-y
        const notesDiv = contentDiv ? contentDiv.nextElementSibling : null; // div.border-t
        
        if (isMobile) {
          // طي الأقسام تلقائياً على الأجهزة المحمولة
          if (contentDiv) contentDiv.classList.add('hidden');
          if (notesDiv) notesDiv.classList.add('hidden');
          arrow.style.transform = 'rotate(90deg)';
        } else {
          // إبقاء الأقسام مفتوحة على سطح المكتب
          arrow.style.transform = 'rotate(0deg)';
        }

        // 4. تفعيل وظيفة الطي والفتح (الأكورديون)
        header.addEventListener('click', (e) => {
          if (e.target.closest('.no-print')) return; // تجاهل النقر على أي زر
          
          if (contentDiv) contentDiv.classList.toggle('hidden');
          if (notesDiv) notesDiv.classList.toggle('hidden');
          arrow.style.transform = contentDiv.classList.contains('hidden') ? 'rotate(90deg)' : 'rotate(0deg)';
        });
      }
    });

    // تطبيق الطي على أقسام التوقيع (للأجهزة المحمولة فقط)
    const isMobileDevice = window.matchMedia('(max-width: 1279px)').matches;
    
    document.querySelectorAll('.print-sig-grid .bg-gray-100').forEach(header => {
      // تحويل العنوان ليكون مرناً وقابلاً للنقر
      header.classList.remove('text-center');
      header.classList.add('flex', 'items-center', 'justify-start', 'cursor-pointer', 'select-none');
      
      // إعادة هيكلة المحتوى لإضافة السهم
      const originalText = header.textContent.trim();
      header.innerHTML = '';
      
      const arrow = document.createElement('span');
      arrow.innerHTML = '▼';
      arrow.className = 'text-gray-500 text-xs transition-transform duration-300 transform ml-2 print:hidden';
      header.appendChild(arrow);
      
      // إضافة الأيقونة
      if (sectionIcons[originalText]) {
        const iconSpan = document.createElement('span');
        iconSpan.textContent = sectionIcons[originalText];
        iconSpan.className = 'ml-2 inline-block text-xl';
      ...
      // ضمان الظهور عند الطباعة
      if (contentDiv) {
        contentDiv.classList.add('print:flex');
        // جعل القسم مطوياً افتراضياً فقط على الأجهزة المحمولة
        if (isMobileDevice) {
  ...
  // تحديث الحالة الأولية للجداول (بعد إنشاء العدادات)
  document.querySelectorAll('table').forEach(updateTableStatus);
});
