...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (printBtn) printBtn.addEventListener('click', () => validateAndAction(() => {
    ...
    // تغيير عنوان الصفحة ليكون رقم اللوحة (اسم الملف الافتراضي عند الطباعة)
    const originalTitle = document.title;
    const plateInput = document.querySelector('input[data-field="plate-number"]');
    if (plateInput && plateInput.value.trim()) {
      document.title = plateInput.value.trim();
    }
