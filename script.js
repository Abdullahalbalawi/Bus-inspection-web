...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const shareBtn = document.querySelector('button[title="مشاركة"]');
  ...
  if (shareBtn) shareBtn.addEventListener('click', () => validateAndAction(shareReport));
  ...
  if (signatureModalCanvas) {
    ...
    function addPoint(pt) {
      ...
      if (dist2 < 25) return false; // زيادة فلتر التنعيم (Smoothing) لتقليل الاهتزازات وجعل الخط انسيابياً
      points.push(pt);
      return true;
    }
  ...
  async function shareReport() {
    ...
    const scoreEl = document.querySelector('.text-blue-600 + span');
    const score = scoreEl ? scoreEl.textContent : '0%';
    
    let text = `📋 *تقرير فحص حافلة*\n`;
    text += `🚌 رقم اللوحة: ${plateNumber}\n`;
    text += `📅 التاريخ: ${date}\n`;
    text += `📊 النتيجة النهائية: ${score}\n\n`;
    
    text += `*ملخص الأقسام:*\n`;
    document.querySelectorAll('table').forEach(table => {
      const title = table.querySelector('th[colspan]')?.innerText.trim();
      const badge = table.querySelector('.percent-badge')?.value;
      if (title && badge) {
        text += `🔹 ${title}: ${badge}\n`;
      }
    });
    ...
    document.querySelectorAll('input[type="checkbox"]').forEach((cb) => {
      if (cb.checked) {
        const row = cb.closest('tr') || cb.closest('.p-4');
        if (row) {
           const checkboxes = row.querySelectorAll('input[type="checkbox"]');
           // Check if it is the fail checkbox (index 1)
           if (checkboxes.length >= 2 && checkboxes[1] === cb) {
             let itemName = '';
             if (row.tagName === 'TR') itemName = row.cells[0].innerText.trim();
             else itemName = row.querySelector('.font-medium').innerText.trim();
             
             const select = row.querySelector('select');
             const reason = select ? select.value : 'فشل';
             failedItems.push(`${itemName} (${reason})`);
           }
        }
      }
    });
    ...
    if (failedItems.length > 0) {
      text += `\n⚠️ *الملاحظات والمشاكل:*\n`;
      failedItems.forEach(item => text += `❌ ${item}\n`);
    } else {
      text += `\n✅ لا توجد مشاكل مسجلة.\n`;
    }

    text += `\nتم الفحص عبر النظام الرقمي.`;

    if (navigator.share) {
      try {
        await navigator.share({
          title: `تقرير فحص حافلة ${plateNumber}`,
          text: text,
        });
      } catch (err) {
      ...
      try {
        ...
        showToast('تم نسخ التقرير! يمكنك لصقه في واتساب أو البريد.', 'success');
      } catch (err) {
        showToast('عفواً، المتصفح لا يدعم المشاركة.', 'error');
      }
    }
  }
  ...
  // منع التكبير عند النقر المزدوج على الأزرار (تحسين الاستجابة في الجوال)
  document.querySelectorAll('button').forEach(btn => {
  ...
  // تمييز خلفية قسم الملاحظات العامة لتمييزه أكثر
  document.querySelectorAll('.lg\\:hidden .border-t').forEach(div => {
    if (div.querySelector('textarea')) {
      div.classList.remove('bg-gray-50');
      div.classList.add('bg-yellow-50');
      div.style.backgroundColor = '#fffbeb'; // لون أصفر فاتح
    }
  });
  
  // إضافة أيقونة قلم بجانب عنوان الملاحظات العامة في الجوال
  document.querySelectorAll('.lg\\:hidden label').forEach(label => {
    if (label.textContent.trim() === 'ملاحظات عامة') {
